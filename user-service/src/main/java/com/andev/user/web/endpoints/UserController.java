...
public class UserController {
    ...
    @Operation(
            summary = "Search users with RSQL", 
            description = "Search users using RSQL (RESTful Service Query Language) with support for complex filtering. "
                    + "Available fields: email, nickname, status, role.name, createdAt, updatedAt. Examples: "
                    + "'email==*@gmail.com' (email contains gmail), "
                    + "'status==ACTIVE' (active users only), "
                    + "'nickname==*john*' (nickname contains john), "
                    + "'role.name==USER' (users with USER role), "
                    + "'status==ACTIVE;email==*@company.com' (active users from company domain), "
                    + "'createdAt>=2024-01-01;status==ACTIVE' (active users created after date)")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
    ...
    public ResponseEntity<AppResponse<PaginationResponse<UserDto>>> searchUsersWithRsql(
            @Parameter(
                    description = "RSQL query for filtering users", 
                    example = "status==ACTIVE;email==*@gmail.com") 
